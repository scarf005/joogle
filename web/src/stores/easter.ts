...
const JOOGLE_TRIGGERS = ["joogle", "ジューグル"]
...
const JJUGEUL_TRIGGERS = ["쮸글", "쭈글", "jjugeul"]

export type EasterEggResult = "jjugeul" | "joogle" | "character" | null

export function checkEasterEgg(query: string): EasterEggResult {
  const normalized = query.toLowerCase().trim()

  if (JJUGEUL_TRIGGERS.includes(normalized)) {
    return "jjugeul"
  }
