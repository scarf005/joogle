...
test.describe("JOOGLE Index", () => {
  test("should show the jjugeul game on index", async ({ page }) => {
    await page.goto("/")
...
test.describe("Search Functionality", () => {
  ...
  test("should show search results for character name", async ({ page }) => {
    ...
    await expect(resultItem).toContainText("Hoshino")
  })
...
test.describe("Easter Eggs", () => {
  ...
  test("should route 쮸글 search to the mini-game", async ({ page }) => {
    await page.goto("/#/home")
    const searchInput = page.locator('input[type="text"]')

    await searchInput.fill("쮸글")
    await searchInput.press("Enter")

    await expect(page).toHaveURL(/#\/jjugeul/)
    await expect(page.locator(".jjugeul__stage")).toBeVisible()
  })

  test("should increment the 쮸글 counter on click and keyboard press", async ({ page }) => {
    await page.goto("/#/jjugeul")

    const count = page.locator(".jjugeul__counter")
